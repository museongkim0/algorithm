...
for i in range(N):
    deque.push_back(str(i+1))

answer = []
for _ in range(N-1):
    pop_var = deque.pop_front()
    answer.append(pop_var)
    pop_var = int(pop_var)
    if int(M[pop_var-1]) > 0:
        for j in range(int(M[pop_var-1])-1):
            deque.push_back(deque.pop_front())
    else:
        for j in range(abs(int(M[pop_var-1]))):
            deque.push_front(deque.pop_back())
answer.append(deque.pop_back())

...
class BinarySearchTree(ADTBinarySearchTree[T]):
    ...
    def insert(self, data: T) -> None:
        if self.tree.is_empty():
            ...
            return
        existing_node = self.__find_node(self.tree.get_root(), data)
        if existing_node and self.tree.get_data(existing_node) == data:
            existing_node.set_data(data)
        else:
            self.__insert_recursive(self.tree.get_root(), data)

    def __find_node(self, node: 'Node[T]', data: T) -> Optional['Node[T]']:
        if node is None:
            return None
    ...
    def erase(self, data: T) -> Optional[T]:
        node = self.__find_node(self.tree.get_root(), data)
